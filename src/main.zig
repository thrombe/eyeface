...
const Engine = struct {
    ...
    fn init() !@This() {
        ...
        var sc = try Swapchain.init(&ctx, window.extent);
        errdefer sc.deinit();
    ...
    fn deinit(self: *@This()) void {
        self.swapchain.deinit(&self.graphics.device);
    ...
    const Swapchain = struct {
        ...
        pub fn init(ctx: *VulkanContext, extent: vk.Extent2D) !Swapchain {
            return try initRecycle(ctx, extent, .null_handle);
        }

        pub fn initRecycle(ctx: *VulkanContext, extent: vk.Extent2D, old_handle: vk.SwapchainKHR) !Swapchain {
        ...
        pub fn recreate(self: *Swapchain, new_extent: vk.Extent2D, ctx: *VulkanContext) !void {
        ...
        pub fn present(self: *Swapchain, cmdbuf: vk.CommandBuffer, ctx: *VulkanContext) !PresentState {
            // Simple method:
...
const Renderer = struct {
    ...
    fn init(ctx: *Engine.VulkanContext, swapchain: *Engine.Swapchain) !@This() {
        ...
        const command_buffers = blk: {
            ...
            for (cmdbufs, framebuffers) |cmdbuf, framebuffer| {
                ...
                device.cmdBindVertexBuffers(cmdbuf, 0, 1, @ptrCast(&vertex_buffer), &offset);
    ...
    fn present(self: *@This(), swapchain: *Engine.Swapchain, ctx: *Engine.VulkanContext) !Engine.Swapchain.PresentState {
        const cmdbuf = self.command_buffers[swapchain.image_index];

        return swapchain.present(cmdbuf, ctx) catch |err| switch (err) {
...
pub fn main() !void {
    {
        ...
        var renderer = try Renderer.init(&engine.graphics, &engine.swapchain);
        ...
        while (!engine.window.should_close()) {
            ...
            const state = try renderer.present(&engine.swapchain, &engine.graphics);
            _ = state;
